...
public class RiverExtractionWorker {
    final static Logger logger = LoggerFactory.getLogger(RiverExtractionWorker.class);
...
class ChannelFunc implements GearmanFunction {
    ...
    public byte[] work(String function, byte[] data, GearmanFunctionCallback callback) throws Exception {
        try {
            ...
            logger.debug("East: " + min_east + " -> " + max_east);
            logger.debug("North: " + min_north + " -> " + max_north);
            ...
            for (int i = 0; i < ortho_tiles.size(); i++) {
                ...
                logger.debug("Processing tile #" + i + ": " + ortho_tile);
                try {
                    ...
                    logger.debug("Couldn't process file: " + e.toString());
                    e.printStackTrace();
                }
